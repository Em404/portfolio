          ...
          {projects.map((project, index) => {
            return (
              <motion.div
                ...
                custom={index}
                key={index}
                className="flex flex-col items-center py-4 px-4 dark:bg-neutral-800 bg-purple-600 rounded-xl text-neutral-50 custom-box-shadow"
              >
                <img
                  src={project.thumb}
                  alt={project.name}
                  className="rounded-xl shadow-neutral-900 shadow-lg"
                />
                <h3 className="mt-4 font-semibold md:text-lg text-center">
                  {project.name}
                </h3>
                ...
                <div className="flex flex-wrap justify-center mt-4">
                  {project.tags.map((tag, index) => {
                    return (
                      <span
                        key={index}
                        className="dark:bg-purple-600 bg-neutral-50 text-purple-600 dark:text-neutral-50 font-semibold rounded-3xl px-2 me-2 mb-2"
                      >
                        {tag}
                      </span>
                    );
                ...
                <Link to={project.repo}>
                  <svg
                    ...
                    width={40}
                    viewBox="0 0 496 512"
                    className="bg-neutral-50 rounded-full mt-2"
                  >
                    <path
                      fill="#9233ea"
